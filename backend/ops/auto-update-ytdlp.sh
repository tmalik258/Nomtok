...
# Cron uses a minimal PATH; Python/pip live under /usr/local/bin in the backend image
...
notify() {
  ...
  if [[ -n "$SMTP_HOST" && -n "$SMTP_FROM" && -n "$NOTIFY_EMAILS" ]]; then
    /usr/local/bin/python3 /usr/local/bin/notify_email.py "$status" "$message" || true
  fi
}
...
CUR_VER=$(/usr/local/bin/python3 -c "import yt_dlp,sys; sys.stdout.write(getattr(yt_dlp,'__version__','unknown'))" || echo "unknown")
echo "$(timestamp) [INFO] Current yt-dlp version: $CUR_VER" | tee -a "$LOG_FILE"

if /usr/local/bin/update-ytdlp >>"$LOG_FILE" 2>&1; then
  NEW_VER=$(/usr/local/bin/python3 -c "import yt_dlp,sys; sys.stdout.write(getattr(yt_dlp,'__version__','unknown'))" || echo "unknown")
